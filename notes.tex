...
\usepackage{hyperref}
\hypersetup{colorlinks=true, urlcolor=myblue, linkcolor=black}
\urlstyle{same}
...
Differences in magnitudes correspond to ratios in fluxes:
\[
...
large galaxies. \mynotes{Why large galaxies?}
...
\begin{itemize}
    ...
    \item $b_{n} \approx 2n - 0.324$ and is determined from the definition of
        $r_e$ \mynotes{(I assume this is talking about where that particular
...
any given location characterized by the velocity ellipsoid, along with the
...
relation\footnote{see, e.g.\ Hogg, Distance Measures in Cosmology})
...
features in distance maps (``fingers of God'').
...
\paragraph{Spectroscopic redshift} Determining redshift spectroscopically
for distant (faint) galaxies is
...
information, i.e.\ multiband photometry to constrain redshift.
Need to
...
\subsection{Integrated brightness}\label{intb}
Issue: galaxies don't have edges.
\begin{itemize}
